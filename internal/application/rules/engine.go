...
func (e *Engine) Run(ctx context.Context, value metric.Sample) ([]event.Event, error) {
	...
	for _, definition := range definitions {
		...
		matches, message, err := service.check(ctx, definition, value)
		if err != nil {
			return created, fmt.Errorf("evaluate rule %s: %w", definition.ID, err)
		}
		if !matches {
			continue
		}
		...
		if err := e.events.Create(ctx, item); err != nil {
			return created, &StoreError{Operation: "persist evaluation event", Kind: ErrEventStore, Cause: fmt.Errorf("%v", err)}
		}
