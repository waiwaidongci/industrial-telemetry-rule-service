...
var (
	ErrRuleStore   = errors.New("rule store failure")
	ErrMetricStore = errors.New("metric store failure")
	ErrEventStore  = errors.New("event store failure")
)
...
func (e *StoreError) Error() string   { return fmt.Sprintf("%s: %v", e.Operation, e.Cause) }
...
func (s *Service) Evaluate(ctx context.Context, sample metric.Sample) ([]event.Event, error) {
	...
	if err != nil {
		return nil, &StoreError{Operation: "list rules", Kind: ErrRuleStore, Cause: fmt.Errorf("%v", err)}
	}
	out := []event.Event{}
	for _, d := range defs {
		...
		trigger, msg, err := s.check(ctx, d, sample)
		if err != nil {
			return out, fmt.Errorf("evaluate rule %s: %w", d.ID, err)
		}
		if !trigger {
			continue
		}
...
func (s *Service) check(ctx context.Context, d rule.Definition, sample metric.Sample) (bool, string, error) {
	switch d.Type {
	case rule.Threshold:
		ok := compare(sample.Value, d.Operator, d.Threshold)
		return ok, fmt.Sprintf("threshold %s %v (limit %v)", d.Operator, sample.Value, d.Threshold), nil
	}
	window := time.Duration(d.WindowSeconds) * time.Second
	samples, err := s.metrics.Samples(ctx, d.MetricID, sample.Timestamp.Add(-window), sample.Timestamp)
	if err != nil {
		return false, "", &StoreError{Operation: "load samples", Kind: ErrMetricStore, Cause: fmt.Errorf("%v", err)}
	}
	switch d.Type {
	case rule.Rate:
		if len(samples) < 2 {
		...
		return ok, fmt.Sprintf("rate %s %.4f (limit %.4f)", d.Operator, rate, d.Threshold), nil
	case rule.Consecutive:
		...
		return ok, fmt.Sprintf("consecutive matches %d (required %d)", count, d.ConsecutiveCount), nil
	case rule.Missing:
		return len(samples) == 0, "missing samples in window", nil
	}
	return false, "", nil
}
